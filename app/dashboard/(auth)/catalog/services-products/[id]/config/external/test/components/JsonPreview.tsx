...
import { Copy, Download, CheckCircle, AlertCircle, PlayCircle, Loader2, MapPin } from "lucide-react";
import { useState } from "react";
import { ResponseMappingsPreview } from "./ResponseMappingsPreview";
import { ExternalServiceSimulation } from "./ExternalServiceSimulation";
...
export function JsonPreview({ 
  ...
  return (
    <div className="space-y-4 min-w-0 w-full">
        ...
        {!hasData ? (
          ...
          <>
            ...
            <div className="bg-muted/30 rounded-lg p-4 border">
              <pre className="text-sm font-mono whitespace-pre-wrap overflow-x-auto max-w-full break-words">
                {jsonString}
              </pre>
            </div>
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              ...
              {Object.entries(data).map(([key, value]) => (
                <div key={key} className="flex justify-between gap-2 min-w-0">
                  <code className="font-mono truncate flex-shrink-0">{key}</code>
      ...
      {/* Mappings attendus (avant simulation) */}
      {endpointId && hasData && !response && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <MapPin className="h-4 w-4" />
            <h4 className="font-medium">Mappings attendus</h4>
          </div>
          <ResponseMappingsPreview
      ...
      {endpointId && hasValidData && (
        <ExternalServiceSimulation
          endpointId={endpointId}
          requestData={data}
          disabled={disabled}
        />
